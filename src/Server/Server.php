...
use Casperlaitw\SSO\Legacy\LegacyMode;
use Jasny\Immutable;
...
class Server
{
    ...
    protected function getVerificationCode(string $brokerId, string $token, string $sessionId): string
    {
        ...
        return gmp_strval(gmp_init(hash('sha256', $brokerId . $token . $sessionId), 16), 36);
    }
    ...
    protected function generateChecksum(string $command, string $brokerId, string $token): string
    {
        ...
        return gmp_strval(gmp_init(hash_hmac('sha256', $command . ':' . $token, $secret), 16), 36);
    }
	...
	$key = $this->getCacheKey($brokerId, $token);
        $cached = $this->cache->set($key, $this->session->getId(), 86400);
